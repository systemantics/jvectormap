...
jvm.WorldMap.prototype = {
  ...
  setFocus: function(scale, centerX, centerY){
    var bboxWidth = -1, bboxHeight, bboxX, bboxY;
    ...
    if (jvm.$.isArray(scale)) {
      for (var i in scale) {
        ...
        if (bboxWidth == -1) {
          bboxWidth = thisBbox.width;
          bboxHeight = thisBbox.height;
          bboxX = thisBbox.x;
          bboxY = thisBbox.y;
        } else {
          var newX = Math.min(bboxX, thisBbox.x);
          bboxWidth = Math.max(bboxX + bboxWidth, thisBbox.x + thisBbox.width) - newX;
          bboxX = newX;
          var newY = Math.min(bboxY, thisBbox.y);
          bboxHeight = Math.max(bboxY + bboxHeight, thisBbox.y + thisBbox.height) - newY;
          bboxY = newY;
        }
      }
    }
    if (bboxWidth==-1) {
      ...
      this.setScale(
        Math.min(this.width / bboxWidth, this.height / bboxHeight),
        - (bboxX + bboxWidth / 2),
        - (bboxY + bboxHeight / 2),
        true
      );
